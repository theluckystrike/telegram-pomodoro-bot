...
const APP_KEYS = ["app_title","app_start","app_stop","app_running","app_minutes","app_loading","app_moreApps","app_shareChat","app_shareStory","app_storyText","app_shareFail","app_errNoInit","app_errExpired","app_errBadSig","app_unlockPro","app_proOneTime","app_proMonthly","app_payDone","app_payCancelled","app_payFailed"];
/** app_* keys rendered as a button: Telegram truncates a long label on narrow screens. No
 * app_proMonthly here -- this bot has no monthly plan (allowMonthly:false), so renderPro
 * never renders that button. */
const BUTTON_KEYS = ["app_shareChat","app_shareStory","app_stop","app_minutes","app_unlockPro","app_proOneTime"];
...
const OPTS = { tokens: [TOKEN], botLink: "https://t.me/Bot", allowMonthly: false };
...
test("pro-link: a real user gets the minted link; no monthly plan degrades to one-time", async () => {
  ...
  // focus has no subscription: allowMonthly:false means "monthly" degrades to one-time.
  const sub = await handleProLink({ initData, plan: "monthly" }, { ...OPTS, mint });
  assert.equal(((await sub.json()) as { url: string }).url, "https://t.me/$onetime");
  assert.deepEqual(seen, ["onetime", "onetime"]);
